    ...
    <div className="carousel-food">
      ...
      <div className="carousel-food__navigator">
        <Button variant="light" onClick={() => prev()}>
          <img src={isFirstPage ? arrowLeftDisabled : arrowLeft} alt="arrow-left"/>
        </Button>
        <Button variant="light" onClick={() => next()}>
          <img src={isLastPage ? arrowRightDisabled : arrowRight} alt="arrow-right"/>
        </Button>
      </div>

      <div className="carousel-food__row">
        <Row ref={carouselDiv} style={{transform: `translate(${translateX}px, 0px)`}}>
          {items.map(item => (
            <Col md={4} className="carousel-food__row--item" key={item.id}>
              <Figure>
                ...
                <Figure.Caption>
                  ...
                  <p>
                    <span>$</span>•
                    <span>{item.cuisineType.title}</span>•
                    <span>{item.mealType.title}</span>•
                    <span>{item.menuType.title}</span>
                  </p>

                  <Badge variant="light">{item.preparation}-{item.preparation + 10} min</Badge>
                  <Badge variant="light">{item.rate} <img src={starIcon} alt="start-icon"/></Badge>
